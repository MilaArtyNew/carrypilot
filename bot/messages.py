...
def format_opportunity(opp: Opportunity, margin_usd: Decimal, leverage: int, paper: bool = False) -> str:
    ...
    return (
        f"{paper_tag}🔍 <b>Funding setup found</b>\n\n"
        ...
        f"Spread: <b>{opp.spread:.4%}</b>\n"
        f"Net: ~<b>{opp.net_profit:.4%}</b>\n\n"
        f"<b>Size:</b>\n"
        f"{qty} {opp.symbol} / {qty} {opp.symbol}\n"
        f"~${usd_short:.0f} / ~${usd_long:.0f}\n\n"
        f"Leverage: up to {leverage}x\n"
        f"Time to funding: {opp.minutes_to_funding:.0f} min"
    )
...
def format_trade_result(result, action: str = "opened", paper: bool = False, trade=None) -> str:
    ...
    if result.success:
        ...
        if trade is not None and trade.realized_pnl:
            ...
            if trade.price_pnl and trade.funding_pnl:
                ...
                text += (
                    f"\n  ├ Price delta: {'+' if p>=0 else ''}{p:.4f}"
                    f"\n  └ Funding: +{f_:.4f}"
                )
        return text
    return (
        f"❌ {paper_tag}<b>Error {action} {result.symbol}</b>\n"
    ...
    for t in reversed(real[-10:]):
        ...
        if pnl is not None:
            ...
            if t.price_pnl and t.funding_pnl:
                ...
                pnl_str += f" (price {'+' if p>=0 else ''}{p:.4f} / funding +{f_:.4f})"
        else:
            pnl_str = "—"
        lines.append(
...
def format_paper_stats(stats: dict) -> str:
    ...
    return (
        f"<b>📄 Paper trading statistics:</b>\n\n"
        f"Open positions: {stats['open']}\n"
        f"Closed trades: {stats['closed']}\n"
        f"Wins / Losses: {stats['wins']} / {stats['losses']}\n"
        f"Win rate: {stats['win_rate']}\n\n"
        f"Total P&L: <b>{sign}{pnl:.4f} USD</b>\n"
        f"  ├ Price delta: {p_sign}{price_pnl:.4f} USD\n"
...
def format_live_stats(stats: dict, recent: list) -> str:
    ...
    lines = [
        ...
        f"Total P&L: <b>{sign}{pnl:.4f} USD</b>",
        f"  ├ Price delta: {p_sign}{price_pnl:.4f} USD",
        f"  └ Funding (estimate): +{funding_pnl:.4f} USD",
    ]
    if recent:
        lines.append("\n<b>Recent trades:</b>")
        for t in recent:
...
def format_emergency(symbol: str, short_ex: str, long_ex: str, short_ok: bool, long_ok: bool) -> str:
    return (
        f"🚨 <b>ERROR: one leg failed to open</b>\n\n"
        f"Symbol: {symbol}\n"
        f"SHORT ({short_ex}): {'✅ opened' if short_ok else '❌ not opened'}\n"
        f"LONG ({long_ex}): {'✅ opened' if long_ok else '❌ not opened'}\n\n"
